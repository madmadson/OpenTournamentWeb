...
export class ActualTournamentRankingService {
    ...
    query.subscribe((rankingRef: any) => {
      rankingRef.forEach((ranking) => {
        if (ranking.tournamentRound === config.round) {
          this.afoDatabase.object('tournament-games/' + config.tournamentId + '/' + ranking.$key).remove();
        }
      });
    });
  }

  pushRankingForRound(allRankings: TournamentRanking[], config: TournamentManagementConfiguration): TournamentRanking[] {
  ...
  updateRankingAfterGameResultEntered(rankingUpdateModel: RankingUpdateModel) {

    const gameResult = rankingUpdateModel.gameResult;
    ...
    for (let i = roundOfGameResult; i <= rankingUpdateModel.actualTournament.actualRound; i++) {

      const allRankingsFromSameRound: TournamentRanking[] = _.filter(rankingUpdateModel.allRankings, function (ranking: TournamentRanking) {
      ...
      const lastRoundRankingPlayerOne: TournamentRanking = _.filter(rankingUpdateModel.allRankings, function (ranking: TournamentRanking) {
        return (ranking.tournamentRound === (i - 1) &&
        ranking.tournamentPlayerId === gameResult.gameAfter.playerOneTournamentPlayerId);
      })[0];

      const lastRoundRankingPlayerTwo: TournamentRanking = _.filter(rankingUpdateModel.allRankings, function (ranking: TournamentRanking) {
        return (ranking.tournamentRound === (i - 1) &&
        ranking.tournamentPlayerId === gameResult.gameAfter.playerTwoTournamentPlayerId);
      })[0];

      const rankingPlayerOne: TournamentRanking = _.filter(rankingUpdateModel.allRankings, function (ranking: TournamentRanking) {
        return (ranking.tournamentRound === (i) &&
        ranking.tournamentPlayerId === gameResult.gameAfter.playerOneTournamentPlayerId);
      })[0];

      const rankingPlayerTwo: TournamentRanking = _.filter(rankingUpdateModel.allRankings, function (ranking: TournamentRanking) {
        return (ranking.tournamentRound === (i) &&
        ranking.tournamentPlayerId === gameResult.gameAfter.playerTwoTournamentPlayerId);
      })[0];

      const actualRoundGamePlayerOne: TournamentGame = _.filter(rankingUpdateModel.allGames, function (game: TournamentGame) {
        return (game.tournamentRound === (i) &&
        (game.playerOneTournamentPlayerId === rankingUpdateModel.gameResult.gameAfter.playerOneTournamentPlayerId ||
        game.playerTwoTournamentPlayerId === rankingUpdateModel.gameResult.gameAfter.playerOneTournamentPlayerId));
      })[0];

      const actualRoundGamePlayerTwo: TournamentGame = _.filter(rankingUpdateModel.allGames, function (game: TournamentGame) {
        return (game.tournamentRound === (i) &&
        (game.playerOneTournamentPlayerId === rankingUpdateModel.gameResult.gameAfter.playerTwoTournamentPlayerId ||
        game.playerTwoTournamentPlayerId === rankingUpdateModel.gameResult.gameAfter.playerTwoTournamentPlayerId));
      })[0];

      if (rankingUpdateModel.gameResult.gameAfter.playerOneTournamentPlayerId !== 'bye') {
