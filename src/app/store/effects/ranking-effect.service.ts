...
import {
  SubscribeTournamentRankingsAction,
  TOURNAMENT_RANKINGS_SUBSCRIBE_ACTION
...
export class RankingEffectService {
  ...
  @Effect({dispatch: false}) subscribe = this.actions$
    ...
    .map((action: SubscribeTournamentRankingsAction) => this.rankingService.subscribeOnTournamentRankings(action.payload));
